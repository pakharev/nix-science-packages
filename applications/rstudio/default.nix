...
(conf: {
  RSTUDIO_VERSION_SUFFIX = "+${builtins.replaceStrings [ "-" ] [ "" ] conf.date}";
})

(conf: {
  meta.mainProgram = "rstudio${lib.optionalString conf.server "-server"}";
  version = with conf; 
...
(with commonLocations; resolveLocations {
  ...
  awsMathJax = empty.override (conf: {
    method = "fetchurl";
    name = "aws-mathjax-${conf.mathJaxVersion}-source";
  ...
  total = populateFetchers deps conf;
in with total; (if server then stdenv.mkDerivation else mkDerivation) (total // {
  nativeBuildInputs = [
    cmake
    unzip
    ant
